...
function createChallenge(realm, domain) {
    ...
    var algorithm = 'md5';
    var opaque = '';
    var challenge = 'WWW-Authenticate: Digest realm="' + realm + '", qop="' + qop + '", nonce="' + nonce + '"';
...
function validateAuthorization(authorizationHeader, realm, username, password, method, content) {
    ...
    var digest;
    if(qop === 'auth-int'){
        digest = md5(ha1 + ':' + params.nonce + ':' + numberTo8Hex(nc) + ':' + cnonce + ':' + qop, md5(method + ':' + uri + ':' + md5(content)));
    } else if(qop === 'auth'){
        // console.log('method , params.uri', method, params.uri);
        // console.log('ha1 , params.nonce , numberTo8Hex(nc) , cnonce , qop', ha1 , params.nonce , numberTo8Hex(nc) , cnonce , qop);

        digest = md5(ha1 + ':' + params.nonce + ':' + numberTo8Hex(nc) + ':' + cnonce + ':' + qop + ':' + md5(method + ':' + params.uri));
    } else {
        digest = md5(ha1 + ':' + params.nonce + ':' + md5(method + ':' + params.uri));
    }

    // console.log('digest', digest);
    // console.log('params.response', params.response);
    if (digest === params.response) {
        return true;
    }
