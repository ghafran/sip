...
function Sip(config) {
    ...
    _self.onTcpServerConnection = function (socket) {
        ...
        socket.on('data', function (data) {
            ...
            Runtime.log('tcp data from', socket.remoteAddress, socket.remotePort, sdata);
    ...
    _self.onUdpMessage = function (data, remote) {

        var sdata = data.toString('ascii');
        Runtime.log('udp data from', remote.address, remote.port, sdata);
        var message = new Message(sdata);
        message.isUdp = true;
        message.udpMessage = data;
        _self.handleMessage(message);
    };
    ...
    _self.listen = function () {
        ...
        _self.udpServer = dgram.createSocket(net.isIPv6(_self.config.address) ? 'udp6' : 'udp4');
        _self.udpServer.on('message', _self.onUdpMessage);
        _self.udpServer.bind(_self.config.port, _self.config.address);
        Runtime.log('udp server listening on', _self.config.address, _self.config.port);
    };
