...
function RegisterResponse(message) {
    ...
    _self.generate = function () {
        ...
        // via = via.replace('rport;', 'rport=' + message.header('via').via().port() + ';');
        ...
        if(_self.statusCode === 401){
            ...
            var realm = message.header('to').aof().host();
            var domain = message.header('to').aof().host();
        ...
        var response = startLine + '\r\n' +
            ...
            'Call-ID: ' + callId + '\r\n' +
            'CSeq: ' + cSeq + '\r\n' +
            (_self.statusCode === 200 ? 'Contact: ' + contact + '\r\n' : '') +
            'Content-Length: 0' + '\r\n' +
